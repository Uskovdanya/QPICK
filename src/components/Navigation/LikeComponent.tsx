...
import { getTotalLikeQuantity } from "../../state/likeSlice";
import { useAppSelector } from "../../hooks/hooks";

function LikeComponent() {
  const totalLikeQuantity = useAppSelector(getTotalLikeQuantity);
  return (
    <Link
      to="/liked"
      className="like__container group relative cursor-pointer p-5 hover:-translate-y-[2px] active:translate-y-0"
    >
      <svg
        className="like__image h-5 w-5"
        width="23"
        height="20"
        viewBox="0 0 23 20"
        fill="none"
      ...
      >
        <path
          className="fill-textTitle group-hover:fill-accentColor2 group-active:fill-accentColor"
      ...
      <CountComponent totalCartQuantity={totalLikeQuantity} />
    </Link>
  );
}
